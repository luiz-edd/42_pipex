...
/*   Updated: 2023/12/18 19:58:16 by leduard2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	manage_child(t_pipex *pipex, int cmd_position)
{
	if (cmd_position == 0)
	{
		if (pipex->has_herodoc)
			here_doc(pipex, cmd_position);
		else
			child_first(pipex, cmd_position);
	}
	if (cmd_position < pipex->cmd_quantity - 1)
...
void	here_doc(t_pipex *pipex, int cmd_position)
{
	...
	while (line)
	{
		if (ft_strncmp(line, limiter, ft_strlen(limiter)) == 0)
			exit(EXIT_SUCCESS);
		write(pipex->tube[cmd_position].write_end, line, ft_strlen(line));
...
void	child_limiter(t_pipex *pipex, int cmd_position)
{
	...
	if (dup2(pipex->tube[cmd_position].read_end, STDIN_FILENO) == -1)
	{
		ft_putstr_fd("DUP IN ERROR", 2);
		exit(EXIT_FAILURE);
	}
	if (dup2(pipex->tube[cmd_position + 1].write_end, STDOUT_FILENO) == -1)
	{
...
void	child_middle(t_pipex *pipex, int cmd_position)
{
	...
	if (pipex->has_herodoc)
	{
		cmd_path = pipex->cmd[cmd_position - 1]->cmd_path;
		cmd_args = pipex->cmd[cmd_position - 1]->cmd_args;
	}
	else
	{
		cmd_path = pipex->cmd[cmd_position]->cmd_path;
